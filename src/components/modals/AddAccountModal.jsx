...
export default function AddAccountModal({ isOpen, onClose, onSuccess }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      showToast(t('modal_acc.fail') + e.message, 'error')
    } finally {
      setIsLoading(false)
    }
  }
    ...
    <div className="fixed inset-0 bg-slate-900/60 z-50 flex items-center justify-center p-4 transition-opacity duration-300">
      <div className="bg-white rounded-2xl max-w-sm w-full p-6 shadow-2xl transform transition-transform duration-300 max-h-[90vh] overflow-y-auto">
        <div className="flex justify-between items-center mb-6">
          <h3 className="font-extrabold text-lg text-slate-800">{t('modal_acc.title_new')}</h3>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="block text-xs text-slate-500 mb-1 font-semibold">{t('modal_acc.name_label')}</label>
          ...
          <button type="submit" disabled={isLoading}
            ...
            {isLoading ? t('modal_acc.btn_saving') : t('modal_acc.btn_save')}
          </button>
        </form>
      </div>
    </div>
