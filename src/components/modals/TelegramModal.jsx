...
export default function TelegramModal({ isOpen, onClose, initialSettings, onSettingsChange }) {
  ...
  const handleUnlink = async () => {
    if (!confirm('Yakin ingin memutuskan koneksi dengan Telegram? Anda tidak akan bisa mencatat transaksi via Telegram lagi.')) return
    ...
    try {
      ...
      onSettingsChange(updated)
      showToast('Koneksi Telegram berhasil diputus', 'success')
      setLinkCode('')
      checkOrGenerateCode()
    } catch (e) {
      ...
      <div className="bg-white rounded-3xl w-full max-w-md relative z-10 p-6 shadow-2xl animate-slide-up md:animate-fade-in flex flex-col max-h-[90vh]">
        <div className="flex justify-between items-center mb-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-xl font-extrabold text-slate-800">Telegram Bot</h2>
              <p className="text-[11px] font-bold text-slate-400 uppercase tracking-wider">Integrasi Asisten Pintar</p>
            </div>
          </div>
        ...
        <div className="overflow-y-auto no-scrollbar pb-6 space-y-6 flex-1">
          {isLinked ? (
            <div className="text-center py-6">
              ...
              <h3 className="text-lg font-bold text-slate-800">Akun Terhubung!</h3>
              <p className="text-sm text-slate-500 mt-2 mb-6">Anda sudah bisa mencatat pengeluaran dengan mengirimkan foto nota ke Bot Telegram Anda.</p>
              ...
              >
                ...
                {isLoading ? 'Memproses...' : 'Putuskan Koneksi'}
              </button>
            </div>
          ) : (
            <div>
              <p className="text-sm text-slate-600 mb-6">
                Hubungkan Savora dengan Telegram untuk mencatat transaksi secara otomatis hanya dengan mengirimkan foto nota ke bot Anda.
              </p>
              ...
              <div className="bg-slate-50 p-4 rounded-2xl border border-slate-200 mb-6 relative overflow-hidden">
                ...
                <h4 className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-3">Langkah 1</h4>
                <p className="text-sm text-slate-700 font-medium mb-1">
                  Buka Bot Telegram Savora Anda.
                </p>
                <p className="text-xs text-slate-500">
                  (Pastikan Anda sudah membuat bot via BotFather dan memasukkan Token di Vercel).
                </p>
              </div>

              <div className="bg-blue-50 p-4 rounded-2xl border border-blue-100 relative">
                <h4 className="text-xs font-bold text-blue-500 uppercase tracking-wider mb-3">Langkah 2</h4>
                <p className="text-sm text-blue-900 font-medium mb-3">
                  Kirimkan kode rahasia ini ke obrolan bot tersebut:
                </p>
                ...
                <p className="text-[11px] text-blue-600 mt-3 text-center opacity-80">
                  Setelah mengirim kode, tutup jendela ini lalu muat ulang halaman.
                </p>
              </div>
            </div>
          )}
